...
export default function Terbaru() {
  ...
  let revealDiv4: HTMLDivElement | undefined;

  onMount(() => {
    const observer = new IntersectionObserver(
      entries => {
        entries.forEach(entry => {
          if (entry.isIntersecting && revealDiv4) {
            revealDiv4.classList.remove('opacity-0');
            revealDiv4.classList.add('opacity-100');
            observer.unobserve(entry.target); // Berhenti observe setelah muncul
          }
        });
      },
    ...
    if (revealDiv4) observer.observe(revealDiv4);
  });
  ...
  return (
    ...
    <div class="flex flex-col w-full h-[90vh] mt-20 pb-10">
        <A href="/produk"
        class="relative bg-cover w-full bg-center md:bg-top text-center justify-center flex flex-1 items-center group"
        style="background-image: url('/src/public/images/mockup/9.png');"
        >
        <div class="absolute z-0 inset-0 bg-black/15 opacity-50 duration-700 group-hover:opacity-100"></div>

        <div ref={revealDiv4} class="w-full z-10 opacity-0 duration-700 max-w-sm md:max-w-sm lg:max-w-md p-4 rounded text-white font-medium">
            <h2 class="w-full mb-2 text-3xl lg:text-4xl font-medium lg:font-semibold">Lihat artikel hoodie terbaru Personal Store</h2>
            <div class="hidden md:block">
                ...
                <div class="flex items-center px-4 py-2 text-black hover:bg-gray-100 bg-white rounded-full"><i class="fa-solid fa-bag-shopping mr-1"></i> Shop</div>
                </A>
            </div>
    ...
    <div class="mt-2 mb-12 h-[100vh] flex flex-col">
      <A href="/produk"
        class="relative w-full flex-1 bg-cover bg-center md:bg-top flex items-center group"
        style="background-image: url('/src/public/images/mockup/6.png');"
      >
        ...
        <div ref={revealDiv3} class="opacity-0 translate-x-0 transition-all duration-700 overflow-x-auto w-full max-w-sm md:max-w-sm lg:max-w-md ml-2 md:ml-16 lg:ml-32 rounded text-white font-medium">
          <h2 class="w-full mb-1 text-3xl lg:text-4xl font-medium lg:font-semibold">Artikel Kaos terbaru dari Personal Store</h2>
